...
jQuery(document).ready(function() {
	...
	jQuery(document.body).on('click', '.um-notification:not(.none)', function() {
		...
		if ( notification_uri ) {
			window.open( notification_uri, '_blank' );
		}
	...
	jQuery(document.body).on('mouseenter', '.um-notification:not(.none)',function(e) {
		...
		var icon_book = jQuery(this).find('.b2 .um-faicon-book');

		if( icon_book.length == 0 ) {

			if ( jQuery(this).hasClass('unread') && !jQuery(this).hasClass('viewed') ) { // only if unread

				var notification_id = jQuery(this).attr('data-notification_id');

				jQuery('*[data-notification_id='+notification_id+']').addClass('viewed');
				
				var notification = jQuery(this);
					notification.addClass('viewed');
